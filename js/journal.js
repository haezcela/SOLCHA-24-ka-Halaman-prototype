...
function renderJournalEntry(doc) {
  ...
  if (data.from === "user") {
    ...
    buttonText = "Comment";
    clickAction = `toggleComment('${doc.id}', '${
                        ...
                        <div class="row m-3">
                          <div class="col-3 offset-2">
                            <button type="button" id="${
                              doc.id
                            }-action" class="btn btn-success btn-sm" onclick="${
    data.from !== "user"
      ? `toggleComment('${doc.id}','${data.comment ? data.comment : ""}')`
      : `showEditForm('${doc.id}', '${data.data}')`
  }">${buttonText}</button>
                          </div>
                          <div class="col-3 offset-2">
                            <button type="button" class="btn btn-danger btn-sm" id="${
                              doc.id
                            }-delete" onclick="deleteJournalEntry('${
    doc.id
  }')">Delete</button>
                          </div>
                          <div class="col-2"></div>
                        ...
                        <div id="${
                          doc.id
                        }-comment-section" class="col-12"></div> <!-- Added col-12 for the comment section -->
                    </div>
                  </div>`;
...
document.getElementById("cancelNoteBtn").addEventListener("click", function () {
  ...
  const confirmation = confirm(
    "Are you sure you want to cancel adding the note?"
  );
...
function saveEditedData(docId) {
  const editedData = document
    .getElementById(`${docId}-editTextarea`)
    .value.trim();
  if (editedData !== "") {
    // Disable the button to prevent multiple clicks
